...
const ProjectTaskList = ({ tasks, setTasks, selectedTasks, setSelectedTasks, formatTaskDisplay }) => {
  ...
  const removeAirthingsDevice = async(payload) => {
    const response = await fetch("https://rykjmxrsxfstlagfrfnr.supabase.co/functions/v1/remove_airthings_device", {
      method: "DELETE",
      headers: {
      ...
      <Modal isOpen={isUnlinkModalOpen} onClose={closeUnlinkModal}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            ...
            <Text fontSize="sm">
              Remember to manually <Link href={`https://dashboard.airthings.com/devices/${currentDevice?.at_serialNumber}`} textDecoration="underline" isExternal>Unpair Device in the Airthings dashboard</Link> .
            </Text>
          </ModalBody>
          <ModalFooter>
